...
import { ComposedChart, Area, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell, BarChart, LineChart, Line } from 'recharts';
...
const Projecthome = ({ projectid, onSlectedProject, graphLoading }) => {
  ...
  const processIssuesData = useCallback((issues) => {
    ...
    const processedData = issues.map((issue) => {
      ...
      const startDate = issue.fields.created || null;
    ...
    if (storedProjectId && userToken) {
      try {
        ...
        const response = await axios.get(`http://localhost:4000/api/tasks?projectId=${storedProjectId}&startAt=0&maxResults=1`, {
          headers: { Authorization: `Bearer ${userToken}` } // Include the token in the request
        });
  ...
  const taskCompletionData = useMemo(() => {
    ...
    projectTasks.forEach(task => {
      ...
      if (completionDate) {
        ...
        counts[completionDate] = (counts[completionDate] || 0) + 1;
      }
    });
    ...
    dates.forEach(date => {
      cumulativeCount += counts[date];
      cumulativeCounts.push(cumulativeCount);
    });

    return { dates, cumulativeCounts };
  }, [projectTasks]);
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      {isGraphLoading || isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-0 bg-black bg-opacity-40 backdrop-filter backdrop-blur-sm rounded-lg pt-1 overflow-y-scroll scrollbar-hide">
            ...
            <div className="overflow-x-scroll scrollbar-hide">
              {isGraphLoading ? (
                ...
                <div style={{ width: '1000px' }}>
                  <ResponsiveContainer width={graphData.length * 25}  height={300}>
                    <ComposedChart
                      data={graphData}
                      margin={{
                        top: 0,
          ...
          <div className="col-span-1 sm:col-span-2 md:col-span-3 lg:col-span-5 bg-transparent p-4 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold text-center mb-4 text-white">Task Completion Over Time</h2>
            <div className="overflow-x-scroll scrollbar-hide">
              {isGraphLoading ? (
                <div className="h-64 w-full bg-gray-300 rounded-md animate-pulse"></div>
              ) : (
                <ResponsiveContainer width="100%" height={300}>
                  <ComposedChart
                    data={taskCompletionData.dates.map((date, index) => ({
                      date,
                      cumulativeCount: taskCompletionData.cumulativeCounts[index],
                    }))}
                    margin={{
                      top: 0,
                      right: 20,
                  ...
                  >
                    <CartesianGrid stroke="#1c2d41" />
                    <XAxis stroke="#fffafa" dataKey="date" />
                    <YAxis stroke="#fffafa" />
                    <Tooltip />
                    <Legend />
                    <Area type="monotone" dataKey="cumulativeCount" fill="#FF6384" stroke="#FF6384" />
                  </ComposedChart>
                </ResponsiveContainer>
              )}
            </div>
